...
# Note that: evaluation_control_colored_attractor.png,
# evaluation_controlled_state_path.png,
# evaluation_control_correction.png, and evaluation_rewards.png
# are not generated by this script.
# The code has been removed to avoid cluttering the output with additional figures.
...
def plot_evaluation_diagnostics(
    ...
    *,
    ...
    initial_condition_label = np.array2string(
    ...
    trajectory_axis.set_title(
        f"Evaluation trajectory from fixed initial condition "
        f"{initial_condition_label}"
    )
    if label_trials:
    ...
    state_figure.suptitle(
        f"State response from fixed initial condition {initial_condition_label}\n"
        "Greedy Q-table evaluation with no initial-state perturbation"
    )
